...
class UclidContext():
    ...
    def __add_typdecl__(name, decl):
        if name in UclidContext.typ_decls[UclidContext.module_name]:
            logging.warn("Redeclaration of type named {}".format(name))
        else:
    ...
    def __add_vardecl__(name, decl):
        if name in UclidContext.var_decls[UclidContext.module_name]:
            logging.warn("Redeclaration of name {}".format(name))
        else:
    ...
    def __add_constdecl__(name, decl):
        if name in UclidContext.const_decls[UclidContext.module_name]:
            logging.warn("Redeclaration of const {}".format(name))
        else:
    ...
    def __constdecls__(module):
        if module.name not in UclidContext.const_decls or not UclidContext.const_decls[module.name]:
            return ""
        return "\n".join([textwrap.indent(decl.__inject__(), '\t')
    ...
    def __importdecls__(module):
        ...
        return "\n".join([textwrap.indent(decl.__inject__(), "\t") for
            decl in UclidContext.import_decls[module.name]])
    ...
    def __definedecls__(module):
        ...
        return "\n".join([decl.__inject__() for
            decl in UclidContext.define_decls[module.name]])
    ...
    def __proceduredefns__(module):
        ...
        return "\n".join([textwrap.indent(defn.__inject__(), "\t") for
            defn in UclidContext.procedure_defns[module.name]])
    ...
    def __moduleassumes__(module):
        ...
        return "\n".join([textwrap.indent(assm.__inject__(), "\t") for
            assm in UclidContext.module_assumes[module.name]])
...
class UclidModule(UclidElement):
    ...
    def __inject__(self):
        ...
        prop_code = '\n'.join([textwrap.indent(prop.__inject__(), "\t") for prop in self.properties])
        acc += textwrap.dedent("""
            module {} {{
            {}
...
init {{
{}
}}'''.format(textwrap.indent(self.block.__inject__(), '\t'))
...
class UclidDecl(UclidElement):
    def __init__(self, obj, decltype) -> None:
        ...
        if self.decltype == DeclTypes.VAR:
            ...
            self.inject_str = "{} {} : {};".format(self.porttype.name, self.name, self.declstring)
...
class UclidArrayType(UclidType):
    def __init__(self, itype, etype, name=""):
        ...
        self.elemtype = etype
        self.declstring = "[{}]{}".format(self.indextype.__inject__(), self.elemtype.__inject__())
        if name != "":
            self.name = name
            self.decl = UclidTypeDecl(self)
        else:
...
class UclidProcedureType(UclidType):
    ...
    def __inject__(self):
        input_sig = ', '.join(["{} : {}".format(i[0] if isinstance(i[0], str) else i[0].lit,  i[1].declstring) for i in self.ip_args])
        modify_sig = "\nmodifies {};".format(
...
class UclidDefine(UclidElement):
    ...
    """
    def __init__(self, name: str, function_sig: UclidFunctionType, body : UclidExpr) -> None:
        super().__init__()    
        self.name = name
