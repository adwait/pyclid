...
class VFormal:
    ...
    @staticmethod
    def __decls__ ():
        acc = "".join([decl.__inject__() 
            for k, decl in VFormal.regs.items()])
        acc += "".join([decl.__inject__() 
            for _, decl in VFormal.wires.items()])
        return acc
    @staticmethod
    def __defns__ ():
        return "".join([defn.__inject__() 
            for _, defn in VFormal.definitions.items()])
    @staticmethod
    def __stmts__ ():
        return "\n".join([sseq.__inject__()
            for _, sseq in VFormal.stmts.items()])
    @staticmethod
    def clear_all():
        VFormal.definitions = {}
        VFormal.declarations = {}
        VFormal.wires = {}
        VFormal.regs = {}
        VFormal.stmts = {}
...
class VBVConst(VExpr):
    ...
    def __inject__(self) -> str:
        if self.width == 0:
            ...
            return "{}'{}{}".format(
                self.width, self.base.__inject__(), self.val
            )
...
class VTernary(VExpr):
    ...
    def __inject__(self) -> str:
        return "({}) ? ({}) : ({})".format(
            self.ifcond.__inject__(), self.texpr.__inject__(), self.eexpr.__inject__())
...
class VSlice(VExpr):
    ...
    def __init__(self, elem : VExpr, hi, lo):
        ...
        self.elem = elem
    def __inject__(self):
        return "{}[{}:{}]".format(self.elem.__inject__(), self.hi, self.lo)
...
class VSignExtend(VExpr):
    ...
    def __inject__(self):
        extension = "{{ {} {{ {}[{}] }} }}".format(
            self.newwidth-self.currwidth, self.elem.__inject__(), self.currwidth-1)
        return "{{ {{ {} }}, {}  }}".format(extension, self.elem.__inject__())
...
class VArraySelect(VExpr):
    ...
    def __inject__(self) -> str:
        return "{}[{}]".format(self.array.__inject__(), ']['.join([ind.__inject__() for ind in self.indices]))
...
class VFuncApplication(VExpr):
    def __init__(self, func : str, args):
        ...
        self.args = args
    def __inject__(self) -> str:
        return "{}({})".format(self.func, ', '.join([arg.__inject__() for arg in self.args]))
    ...
    def __inject__(self):
        if self.vartype == ElemType.REG:
            ...
            return "wire [{}:0] {} {} {};\n".format(
                ...
                self.name,
                "[0:{}]".format(self.type.size-1) if self.type.size > 1 else "",
                ("= " + self.expr.__inject__()) if self.expr is not None else ""
            )
        else:
...
'''
    ...
    Cached in definitions Symtab table in VFormal.
'''
class VDefine (VStatement):
    def __init__(self, name_, value_, width_) -> None:
        super().__init__()
        self.name = name_
        self.value = value_
        self.width = width_
        if self.name in VFormal.definitions:
            logging.warn("Redefinition of macro")
        else:
            VFormal.definitions[self.name] = self
    
    def __inject__(self):
        return "`define {} ({}'b{})\n".format(
            self.name,
            self.width,
            bin(self.value)[2:].zfill(self.width)
        )
...
'''.format(
    self.cond.__inject__(),
    self.stmtseq_t.__inject__(),
    self.stmtseq_e.__inject__()
)
        else:
            return '''
if ({}) begin
...
class VSeqElement ():
    ...
    def __init__(self, stmtseq_, edge_=VEdge.POSEDGE, trigger_ : VExpr = VSignal(name_="clock")) -> None:
        ...
        VFormal.stmts[VSeqElement.uidGen] = self
...
'''.format(
            self.edge.__inject__(),
            self.trigger.__inject__(),
            self.stmtseq.__inject__(),
        )

'''
    Compile the entire FORMAL code block from the VFormal context.
'''
def get_formal_block () -> str:
    acc = ""
    acc += "`ifdef FORMAL\n"
    acc += VFormal.__decls__()
    acc += VFormal.__defns__()
    acc += VFormal.__stmts__()
    acc += "`endif\n"
    return acc
